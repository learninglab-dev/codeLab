...
function csvToJson(file) {
  var theCsv = fs.readFileSync(file, "utf-8");
  var parsedCsv = parse(theCsv, {columns: true});
  // console.log(JSON.stringify(parsedCsv, null, 4));
  return parsedCsv;
}
...
function findOnMongo(theShoot, cb){
  MongoClient.connect(process.env.MONGODB_URL, function(err, db) {
    ...
    db.collection('fcpxmls').find({'shootId': theShoot}).toArray(function(err, docs) {
    ...
    theArray.forEach(function(fcpxml){
      console.log("inserting xml: " + fcpxml.shootId);
      db.collection('fcpxmls').insertOne({fcpxml});
    });
    db.close();
    });
}
...
function io2s(csvFile){
  ...
  var filename = path.basename(csvFile, '.csv');
  // TODO: replace these temp paths when in production
  theJob.jsonPath = path.join(process.env.BASE_DIR, 'io2s', 'json', (filename + "_" + theDate.getTime() + '.json'));
  theJob.xmlPath = path.join(process.env.BASE_DIR, 'io2s', 'xml', (filename + "_" + theDate.getTime() + '.xml'));
  theJob.xmlsRequired = [];
  var offset = 0;
  io2sRequest.forEach((element)=>{
    if (!theJob.xmlsRequired.includes(element.shootId)) {
      theJob.xmlsRequired.push(element.shootId);
      console.log('just pushed in' + element.shootId);
    }
    else {
      console.log('already looking for ' + element.shootId);
    }
  });
  theJob.xmlsRequired.forEach((xmlRequest)=>{

  })
}

function io2sOld(csvFile){

  //// TODO: define xmlData from MongoDB
